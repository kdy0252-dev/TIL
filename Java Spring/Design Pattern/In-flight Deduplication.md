...
public final class InFlightDeduplicator<K, V> {
    ...
    public CompletionStage<V> execute(K key, Supplier<V> task) {
        ...
        return Optional.ofNullable(existing)
                       .<CompletionStage<V>>map(CompletableFuture::minimalCompletionStage)
                       .orElseGet(() -> submitLeader(key, candidate, task));
    }

    private CompletionStage<V> submitLeader(
            K key,
            CompletableFuture<V> candidate,
            Supplier<V> task
    ) {
        return Try.run(() -> executor.execute(() -> complete(key, candidate, task)))
                  .fold(
                      cause -> completeRejectedSubmission(key, candidate, cause),
                      ignored -> candidate.minimalCompletionStage()
                  );
    }

    private void complete(K key, CompletableFuture<V> candidate, Supplier<V> task) {
        Try.of(task::get)
           .fold(candidate::completeExceptionally, candidate::complete);
        inFlight.remove(key, candidate);
    }

    private CompletionStage<V> completeRejectedSubmission(
            K key,
            CompletableFuture<V> candidate,
            Throwable cause
    ) {
        inFlight.remove(key, candidate);
        candidate.completeExceptionally(cause);
...
private static void await(CountDownLatch latch) {
    Try.run(latch::await)
       .onFailure(InterruptedException.class, ignored -> Thread.currentThread().interrupt())
