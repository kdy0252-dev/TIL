...
![[3. Helm Chart 설치 - 01.png]]
![[3. Helm Chart 설치 - 02.png]]

Repository URL, 인증 정보와 연결 방식을 입력한 뒤 **CONNECT**를 누른다. Connection Status가 `Successful`인지 확인한다.

Applications -> **NEW APP**에서 다음 항목을 설정한다.

- Application Name과 Project
- Sync Policy
- Git Repository URL과 Revision
- Helm Chart가 있는 Path
- Destination Cluster와 Namespace

Repository가 정상 연결되면 Source Type이 Helm으로 인식되고 Values와 Parameter를 설정할 수 있다. 생성 전에 Manifest Preview에서 Namespace와 Image Repository를 확인한다.
### ArgoCD Image Updater 연동
Application -> DETAILS  클릭후 나오는 창에서 EDIT 클릭
![[3. Helm Chart 설치 - 03.png]]
아래와 같이 어노테이션을 추가한다.
![[3. Helm Chart 설치 - 04.png]]
어노테이션은 아래 표와 같다.
