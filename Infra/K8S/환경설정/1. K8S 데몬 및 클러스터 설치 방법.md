...
## 가상머신에 설치 시 
### 1. Rocky Linux 설치
Rocky Linux Installer에서 다음 항목을 순서대로 설정한다.

1. **설치 목적지**에서 Kubernetes용 Disk를 선택한다.
2. **Network & Host Name**에서 Network를 활성화하고 고정 IPv4를 설정한다.
3. Control Plane Node의 Host Name을 `k8s-master`로 지정한다.
4. Root Password와 운영용 사용자를 설정하고 SSH 접속을 허용한다.
5. 설치 후 재부팅하여 IP, Host Name과 시간대가 기대한 값인지 확인한다.
