...
List<PassengerResource> resources = passengers.stream()
...
List<PassengerId> passengerIds = bookings.stream()
...
```java
// Stream 안에서 ID별 Repository 호출을 반복하면 N+1 I/O가 된다.
// 먼저 Batch 조회한 뒤 Optional Field만 평탄화한다.
List<EmailAddress> notificationEmails = passengerPort.findAllByIds(passengerIds)
                                                     .stream()
...
```java
Stream<BookingResource> pipeline = bookings.stream()
                                           .filter(Booking::isVisible)
                                           .map(BookingResource::from);

// Terminal Operation 전에는 Mapping이 실행되지 않는다.
List<BookingResource> result = pipeline.toList();
```
